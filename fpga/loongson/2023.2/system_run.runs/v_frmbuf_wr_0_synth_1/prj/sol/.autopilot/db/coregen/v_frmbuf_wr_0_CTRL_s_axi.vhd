...
entity v_frmbuf_wr_0_CTRL_s_axi is
generic (
...
end entity v_frmbuf_wr_0_CTRL_s_axi;
...
-- 0x40 : reserved
-- 0x54 : Data signal of frm_buffer3
--        bit 31~0 - frm_buffer3[31:0] (Read/Write)
-- 0x58 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of v_frmbuf_wr_0_CTRL_s_axi is
    ...
    constant ADDR_STRIDE_DATA_0       : INTEGER := 16#20#;
    constant ADDR_STRIDE_CTRL         : INTEGER := 16#24#;
    constant ADDR_VIDEO_FORMAT_DATA_0 : INTEGER := 16#28#;
    constant ADDR_VIDEO_FORMAT_CTRL   : INTEGER := 16#2c#;
    constant ADDR_FRM_BUFFER_DATA_0   : INTEGER := 16#30#;
    constant ADDR_FRM_BUFFER_CTRL     : INTEGER := 16#34#;
    constant ADDR_FRM_BUFFER2_DATA_0  : INTEGER := 16#3c#;
    constant ADDR_FRM_BUFFER2_CTRL    : INTEGER := 16#40#;
    constant ADDR_FRM_BUFFER3_DATA_0  : INTEGER := 16#54#;
    constant ADDR_FRM_BUFFER3_CTRL    : INTEGER := 16#58#;
    ...
    signal int_stride          : UNSIGNED(15 downto 0) := (others => '0');
    signal int_video_format    : UNSIGNED(15 downto 0) := (others => '0');
    signal int_frm_buffer      : UNSIGNED(31 downto 0) := (others => '0');
    signal int_frm_buffer2     : UNSIGNED(31 downto 0) := (others => '0');
    signal int_frm_buffer3     : UNSIGNED(31 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_STRIDE_DATA_0 =>
                        rdata_data <= RESIZE(int_stride(15 downto 0), 32);
                    when ADDR_VIDEO_FORMAT_DATA_0 =>
                        rdata_data <= RESIZE(int_video_format(15 downto 0), 32);
                    when ADDR_FRM_BUFFER_DATA_0 =>
                        rdata_data <= RESIZE(int_frm_buffer(31 downto 0), 32);
                    when ADDR_FRM_BUFFER2_DATA_0 =>
                        rdata_data <= RESIZE(int_frm_buffer2(31 downto 0), 32);
                    when ADDR_FRM_BUFFER3_DATA_0 =>
                        rdata_data <= RESIZE(int_frm_buffer3(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    frm_buffer           <= STD_LOGIC_VECTOR(int_frm_buffer);
    frm_buffer2          <= STD_LOGIC_VECTOR(int_frm_buffer2);
    frm_buffer3          <= STD_LOGIC_VECTOR(int_frm_buffer3);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_STRIDE_DATA_0) then
                    int_stride(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_stride(15 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FRM_BUFFER_DATA_0) then
                    int_frm_buffer(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_frm_buffer(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FRM_BUFFER2_DATA_0) then
                    int_frm_buffer2(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_frm_buffer2(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FRM_BUFFER3_DATA_0) then
                    int_frm_buffer3(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_frm_buffer3(31 downto 0));
                end if;
            end if;
        end if;
    end process;
